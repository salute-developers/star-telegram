    ...
    override fun onCallEvent(event: CallEvent) {
        ...
        when (event) {
            ...
            is CallEvent.EndCall -> {
                ...
                analytics.send(EVENTS_PREFIX + CALL_END_SUFFIX, event.toJson().toString())
                onSumCallReady(true)
            }
...
private fun CallEvent.EndCall.toJson(): JSONObject {
    return toCallEventJson().apply {
        put("framesEncoded", framesEncoded)
        put("framesDroppedByCapturer", framesDroppedByCapturer)
        put("framesDroppedByEncoderQueue", framesDroppedByEncoderQueue)
        put("framesDroppedByRateLimiter", framesDroppedByRateLimiter)
        put("framesDroppedByCongestionWindow", framesDroppedByCongestionWindow)
        put("framesDroppedByEncoder", framesDroppedByEncoder)
    }
}
