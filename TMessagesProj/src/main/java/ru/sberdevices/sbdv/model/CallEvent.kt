...
sealed class CallEvent(val callId: String, val dateCreate: Date = Calendar.getInstance().time) {
    ...
    data class EndCall(
        ...
        val framesDroppedByCapturer: Int,
        val framesDroppedByEncoderQueue: Int,
        val framesDroppedByRateLimiter: Int,
        val framesDroppedByCongestionWindow: Int,
        val framesDroppedByEncoder: Int,
    ) : CallEvent(cid)
