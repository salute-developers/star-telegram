...
public class VoIPToggleButton extends FrameLayout {
    ...
    public VoIPToggleButton(@NonNull Context context) {
        this(context, VoIPFragment.SbdvVoipDimensions.buttonRadius);
    }
    public VoIPToggleButton(@NonNull Context context, float radius) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            textView.setTextSize(TypedValue.COMPLEX_UNIT_DIP, VoIPFragment.SbdvVoipDimensions.buttonTitleTextSize);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (rippleDrawable == null) {
                rippleDrawable = Theme.createSimpleFocusableSelectorCircleDrawable(
                        AndroidUtilities.dp(this.radius),
                        0,
                        Color.BLACK
                );
    ...
    public void setData(int iconRes, int iconColor, int backgroundColor, float selectorAlpha, boolean recreateRipple, String text, boolean cross, boolean animated) {
        ...
        if (rippleDrawable == null || recreateRipple) {
            int[] currentRippleDrawableState = rippleDrawable != null ? rippleDrawable.getState() : null;
            if (Color.alpha(backgroundColor) == 255 && AndroidUtilities.computePerceivedBrightness(backgroundColor) > 0.5) {
                    rippleDrawable = Theme.createSimpleFocusableSelectorCircleDrawable(
                            AndroidUtilities.dp(radius),
                            0,
                            ColorUtils.setAlphaComponent(Color.BLACK, (int) (255 * 0.1f * selectorAlpha))
                    );
                } else {
                    rippleDrawable = Theme.createSimpleFocusableSelectorCircleDrawable(
                            AndroidUtilities.dp(radius),
                            0,
                            ColorUtils.setAlphaComponent(Color.WHITE, (int) (255 * 0.3f * selectorAlpha))
                    );
                }
            rippleDrawable.setCallback(this);
            if (currentRippleDrawableState != null) {
                    rippleDrawable.setState(currentRippleDrawableState);
                }
            }
