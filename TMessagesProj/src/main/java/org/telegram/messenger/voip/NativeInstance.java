...
public class NativeInstance {
    ...
    private static final String TAG = "NativeInstance";
    ...
    private CodecsSelectedCallback codecsSelectedCallback;
    ...
    public interface CodecsSelectedCallback {
        void run(@Nullable VideoCodecInfo encoder, @Nullable VideoCodecInfo decoder);
    }
    ...
    public static NativeInstance make(String version, Instance.Config config, String path, Instance.Endpoint[] endpoints, Instance.Proxy proxy, int networkType, Instance.EncryptionKey encryptionKey, VideoSink remoteSink, long videoCapturer, AudioLevelsCallback audioLevelsCallback, CodecsSelectedCallback codecsSelectedCallback) {
        ...
        instance.codecsSelectedCallback = codecsSelectedCallback;
    ...
    private void onCodecsSelected(VideoCodecInfo encoder, VideoCodecInfo decoder) {
        if (codecsSelectedCallback != null) {
            AndroidUtilities.runOnUIThread(() -> codecsSelectedCallback.run(encoder, decoder));
        }
    }
    ...
    private void onStop(Instance.FinalState state) {
        Log.d(TAG, "onStop(): " + state + ", ThreadId: " + Thread.currentThread().getId());
        finalState = state;
        if (stopBarrier != null) {
    ...
    public Instance.FinalState stop() {
        ...
        stopNative();
        try {
            boolean reachedZero = stopBarrier.await(10, TimeUnit.SECONDS);
            Log.d(TAG, "stop() barrier reached zero: " + reachedZero + ", finalState: " + finalState);
        } catch (Exception e) {
            FileLog.e(e);
        }
        return finalState;
