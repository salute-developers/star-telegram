...
public class AutoDeleteMediaTask {
    ...
    public static void run() {
        ...
        Utilities.cacheClearQueue.postRunnable(() -> {
            ...
            int defaultCacheMb = 200;
            int maxCacheMb = SharedConfig.getPreferences().getInt("cache_limit", defaultCacheMb);
            if (maxCacheMb != Integer.MAX_VALUE) {
                long maxCacheSize;
                if (maxCacheMb == 1) {
                    maxCacheSize = 1024L * 300L;
                } else {
                    maxCacheSize = maxCacheMb * 1024L * 1000L;
                }
                long totalSize = 0;
                for (int a = 0; a < paths.size(); a++) {
