...
public class ContactsController extends BaseController {
    ...
    public void cleanup() {
        Log.d("ContactsController", "cleanup()");
        contactsBook.clear();
    ...
    public void checkAppAccount() {
        Log.d("ContactsController", "checkAppAccount()");
    ...
    public void deleteUnknownAppAccounts() {
        Log.d(TAG, "deleteUnknownAppAccounts()");
        try {
            systemAccount = null;
            AccountManager am = AccountManager.get(ApplicationLoader.applicationContext);
            Account[] accounts = am.getAccountsByType("org.telegram.messenger");
            ...
            for (int a = 0; a < accounts.length; a++) {
                ...
                Log.d(TAG, "account is found: " + found);
                if (!found) {
                    try {
                        Log.d(TAG, "try remove account [" + accounts[a] + "]");
                        am.removeAccount(accounts[a], null, null);
                    } catch (Exception ignore) {
                        Log.w(TAG, "catch exception: [cause:" + ignore.getCause() + ", message: " + ignore.getMessage() + "]");
                    }
                }
            }
        } catch (Exception e) {
    ...
    public void syncPhoneBookByAlert(final HashMap<String, Contact> contacts, final boolean first, final boolean schedule, final boolean cancel) {
        Log.d(TAG, "syncPhoneBookByAlert()");
        Utilities.globalQueue.postRunnable(() -> {
            if (BuildVars.LOGS_ENABLED) {
    ...
    private boolean checkContactsInternal() {
        boolean isPrudentNetworking = SbdvServiceLocator.getConfig().getPrudentNetworking();
        if (isPrudentNetworking) return false;
    ...
    public void readContacts() {
        ...
        Utilities.stageQueue.postRunnable(() -> {
            Log.d(TAG, contacts.size() + " contacts. Loaded: " + contactsLoaded);
    ...
    protected void performSyncPhoneBook(final HashMap<String, Contact> contactHashMap, final boolean request, final boolean first, final boolean schedule, final boolean force, final boolean checkCount, final boolean canceled) {
        boolean isPrudentNetworking = SbdvServiceLocator.getConfig().getPrudentNetworking();
        if (isPrudentNetworking || (!first && !contactsBookLoaded)) {
            return;
        }
        Log.d(TAG, "performSyncPhoneBook()");
        Utilities.globalQueue.postRunnable(() -> {
    ...
    public void loadContacts(boolean fromCache, final long hash) {
        ...
        Log.d(TAG, "loadContacts() after synchronized block");
        if (fromCache) {
            if (BuildVars.LOGS_ENABLED) {
                Log.d(TAG, "load contacts from cache");
            }
            analyticsCollector.onAppEvent(AppEvent.LOAD_CONTACTS_FROM_CACHE);
            getMessagesStorage().getContacts();
        } else {
            if (BuildVars.LOGS_ENABLED) {
                Log.d(TAG, "load contacts from server");
            }
            analyticsCollector.onAppEvent(AppEvent.LOAD_CONTACTS_FROM_SERVER);
            ...
            getConnectionsManager().sendRequest(req, (response, error) -> {
                if (error == null) {
                    ...
                    if (hash != 0 && res instanceof TLRPC.TL_contacts_contactsNotModified) {
                        ...
                        if (BuildVars.LOGS_ENABLED) {
                            Log.d(TAG, "loadContacts(): contacts didn't change from last sync time");
                        }
                        return;
                    } else {
                        Log.d(TAG, "loadContacts(): " + res.saved_count + " changed contacts");
    ...
    public void processLoadedContacts(final ArrayList<TLRPC.TL_contact> contactsArr, final ArrayList<TLRPC.User> usersArr, final int from) {
        ...
        String contactsSourceText = "";
        if (from == 0) {
            contactsSourceText = ": from server";
        } else if (from == 1) {
            contactsSourceText = ": from db";
        } else if (from == 2) {
            contactsSourceText = ": from imported contacts";
        }
        Log.d(TAG, "processLoadedContacts()" + contactsSourceText);
        AndroidUtilities.runOnUIThread(() -> {
...
//                });
    ...
    private void applyContactsUpdates(ArrayList<Long> ids, ConcurrentHashMap<Long, TLRPC.User> userDict, ArrayList<TLRPC.TL_contact> newC, ArrayList<Long> contactsTD) {
        ...
        if (BuildVars.LOGS_ENABLED) {
            Log.d(TAG, "applyContactsUpdates(). Contacts add = " + newC.size() + " delete = " + contactsTD.size());
