...
class Camera2Session implements CameraSession {
  ...
  @Nullable private Rect cropRect;
  ...
  private class CameraStateCallback extends CameraDevice.StateCallback {
      ...
      try {
        Executor executor = new Executor() {
          ...
          public void execute(Runnable command) {
            cameraThreadHandler.post(command);
          }
        };
        ...
        sessionConfiguration.setSessionParameters(createSessionParams(camera));
    ...
    @SuppressLint("NewApi") // CaptureRequest.Key constructor actually exists in SDK 28 but it is not part of public API
    private CaptureRequest createSessionParams(CameraDevice camera) throws CameraAccessException {
      CaptureRequest.Builder builder = camera.createCaptureRequest(CameraDevice.TEMPLATE_RECORD);

      CaptureRequest.Key<int[]> sourceResolutionKey = new CaptureRequest.Key<>(
              "ru.sberdevices.camera.mode.cropScaleSourceResolution", int[].class
      );

      if (cameraCharacteristics.getAvailableSessionKeys() != null && cameraCharacteristics.getAvailableSessionKeys().contains(sourceResolutionKey)) {
        android.util.Size resolution = getCropSourceResolution();
        builder.set(sourceResolutionKey, new int[]{resolution.getWidth(), resolution.getHeight()});
      }

      CaptureRequest.Key<byte[]> dewarpByCropKey = new CaptureRequest.Key<>(
              "ru.sberdevices.camera.mode.dewarpByCrop.enable", byte[].class
      );
      if (cameraCharacteristics.getAvailableSessionKeys() != null && cameraCharacteristics.getAvailableSessionKeys().contains(dewarpByCropKey)) {
        if (getDewarpByCropEnabled()) {
          builder.set(dewarpByCropKey, new byte[] {1});
        }
      }
    ...
    private android.util.Size getCropSourceResolution() {
      return SbdvServiceLocator.getLocalConfigSharedInstance().getLocalConfigStateFlow().getValue().getCropSourceResolution();
    }

    private boolean getDewarpByCropEnabled() {
      return SbdvServiceLocator.getLocalConfigSharedInstance().getLocalConfigStateFlow().getValue().getDewarpByCropEnabled();
    }
  ...
  public void setCropArea(Rect rect) {
    if (Objects.equals(rect, cropRect)) {
      return;
    }
    ...
    try {
      captureSession.setRepeatingRequest(
              Objects.requireNonNull(getCaptureRequest()), new CameraCaptureCallback(), cameraThreadHandler);
    ...
    public void onConfigured(CameraCaptureSession session) {
      ...
      try {
        session.setRepeatingRequest(
            getCaptureRequest(), new CameraCaptureCallback(), cameraThreadHandler);
      } catch (CameraAccessException e) {
        reportError("Failed to start capture request. " + e);
        return;
      }
