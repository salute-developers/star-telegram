...
public class HardwareVideoEncoderFactory implements VideoEncoderFactory {
  ...
  private boolean isHardwareSupportedInCurrentSdkH264(MediaCodecInfo info) {
    if (!SbdvServiceLocator.getConfig().getVoipH264Enabled()) {
      return false;
    }
    String name = info.getName();
  ...
  private boolean isHardwareSupportedInCurrentSdkH265(MediaCodecInfo info) {
    if (SbdvServiceLocator.getLocalConfigSharedInstance().getLocalConfigStateFlow().getValue().getAvoidH265Encoder()) {
      return false;
    }
    String name = info.getName();
  ...
  private BitrateAdjuster createBitrateAdjuster(VideoCodecMimeType type, String codecName) {
    ...
    if (codecName.startsWith(HISI_PREFIX)) {
      // Huawei H264 encoders need dynamic bitrate adjustment.
      if (type == VideoCodecMimeType.H264) {
        return new DynamicBitrateAdjuster();
      }
    }

    if (codecName.startsWith(AMLOGIC_PREFIX)) {
      // Amlogic H265 encoders need framerate-based bitrate adjustment.
