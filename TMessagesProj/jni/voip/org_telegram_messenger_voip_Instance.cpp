...
jobject asJavaVideoStats(JNIEnv *env, const VideoStats &videoStats) {
    jmethodID initMethodId = env->GetMethodID(VideoStatsClass, "<init>", "(IIIIIIIII)V");
    return env->NewObject(
            VideoStatsClass,
            initMethodId,
            (jint) videoStats.input_frame_rate,
            (jint) videoStats.encode_frame_rate,
            (jint) videoStats.avg_encode_time_ms,
            (jint) videoStats.frames_encoded,
            (jint) videoStats.frames_dropped_by_capturer,
            (jint) videoStats.frames_dropped_by_encoder_queue,
            (jint) videoStats.frames_dropped_by_rate_limiter,
            (jint) videoStats.frames_dropped_by_congestion_window,
            (jint) videoStats.frames_dropped_by_encoder);
}
...
jobject asJavaFinalState(JNIEnv *env, const FinalState &finalState) {
    ...
    jobject videoStats = asJavaVideoStats(env, finalState.callStats.videoStats);
    return env->NewObject(FinalStateClass, FinalStateInitMethod, persistentState, debugLog, trafficStats, isRatingSuggested, videoStats);
}
...
void initWebRTC(JNIEnv *env) {
    ...
    VideoStatsClass = static_cast<jclass>(env->NewGlobalRef(env->FindClass("org/telegram/messenger/voip/Instance$VideoStats")));
    FinalStateInitMethod = env->GetMethodID(FinalStateClass, "<init>","([BLjava/lang/String;Lorg/telegram/messenger/voip/Instance$TrafficStats;ZLorg/telegram/messenger/voip/Instance$VideoStats;)V");
}
...
JNIEXPORT jlong JNICALL Java_org_telegram_messenger_voip_NativeInstance_makeNativeInstance(JNIEnv *env, jclass clazz, jstring version, jobject instanceObj, jobject config, jstring persistentStateFilePath, jobjectArray endpoints, jobject proxyClass, jint networkType, jobject encryptionKey, jobject remoteSink, jlong videoCapturer, jfloat aspectRatio) {
    ...
    Descriptor descriptor = {
            ...
            .codecsSelected = [platformContext](const webrtc::SdpVideoFormat& encoder, const webrtc::SdpVideoFormat& decoder) {
                jobject globalRef = ((AndroidContext *) platformContext.get())->getJavaInstance();
                tgvoip::jni::DoWithJNI([globalRef, encoder, decoder](JNIEnv *env) {
                    webrtc::ScopedJavaLocalRef<jobject> j_encoder_ref = webrtc::jni::SdpVideoFormatToVideoCodecInfo(env, encoder);
                    webrtc::ScopedJavaLocalRef<jobject> j_decoder_ref = webrtc::jni::SdpVideoFormatToVideoCodecInfo(env, decoder);
                    jmethodID j_method = env->GetMethodID(NativeInstanceClass, "onCodecsSelected", "(Lorg/webrtc/VideoCodecInfo;Lorg/webrtc/VideoCodecInfo;)V");

                    env->CallVoidMethod(globalRef, j_method, j_encoder_ref.obj(), j_decoder_ref.obj());
                });
            },
