...
MediaManager::MediaManager(
    ...
    std::function<void(webrtc::SdpVideoFormat& encoder, webrtc::SdpVideoFormat& decoder)> codecsSelected,
...
_codecsSelected(std::move(codecsSelected)),
...
void MediaManager::collectStats() {
    ...
    _videoStats = VideoStats {
	    ...
	    stats.frames_dropped_by_capturer,
	    stats.frames_dropped_by_encoder_queue,
	    stats.frames_dropped_by_rate_limiter,
	    stats.frames_dropped_by_congestion_window,
	    stats.frames_dropped_by_encoder,
	};
...
void MediaManager::setPeerVideoFormats(VideoFormatsMessage &&peerFormats) {
    ...
    if (_codecsSelected) {
        webrtc::SdpVideoFormat *myEncoder = nullptr;
        webrtc::SdpVideoFormat *theirEncoder = nullptr;
        if (formats.myEncoderIndex >= 0) {
            myEncoder = &formats.list[formats.myEncoderIndex];
        }
        if (formats.theirEncoderIndex >= 0) {
            theirEncoder = &formats.list[formats.theirEncoderIndex];
        }
        _codecsSelected(*myEncoder, *theirEncoder);
    }
...
void MediaManager::configureSendingVideoIfNeeded() {
    ...
    codec.SetParam(cricket::kCodecParamStartBitrate, 1500);
    codec.SetParam(cricket::kCodecParamMaxBitrate, _enableHighBitrateVideo ? 4000 : 2500);
...
int MediaManager::getMaxVideoBitrate() const {
    return (_enableHighBitrateVideo && _isLowCostNetwork) ? 4000000 : 2500000;
}
...
void MediaManager::adjustBitratePreferences(bool resetStartBitrate) {
    if (computeIsSendingVideo()) {
        ...
        if (_isScreenCapture) {
            ...
            if (resetStartBitrate) {
                preferences.start_bitrate_bps = 1500000;
            }
        }
...
void MediaManager::fillCallStats(CallStats &callStats) {
    ...
    callStats.videoStats = _videoStats;
}
